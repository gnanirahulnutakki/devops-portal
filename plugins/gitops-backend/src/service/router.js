...
import { PermissionService, Role, requireRole } from '../services/PermissionService';
...
export async function createRouter(options) {
    ...
    const githubActionsService = new GitHubActionsService(config, logger);
    ...
    const defaultRole = config.getOptionalString('gitops.permissions.defaultRole') || Role.DEVELOPER;
     ...
     */
    router.get('/repositories/:repo/actions/workflows', asyncHandler(async (req, res) => {
        const { repo } = req.params;
     ...
     */
    router.get('/repositories/:repo/actions/runs', asyncHandler(async (req, res) => {
        const { repo } = req.params;
        ...
        const { runs, total_count } = await githubActionsService.getWorkflowRuns(repo, {
            workflow_id: workflow_id ? parseInt(workflow_id, 10) : undefined,
            branch: branch,
            status: status,
     ...
     */
    router.get('/repositories/:repo/actions/runs/:runId', asyncHandler(async (req, res) => {
     ...
     */
    router.get('/repositories/:repo/actions/runs/:runId/jobs', asyncHandler(async (req, res) => {
        const { repo, runId } = req.params;
     ...
     */
    router.get('/repositories/:repo/actions/summary', asyncHandler(async (req, res) => {
        const { repo } = req.params;
        const { branch } = req.query;
        logger.info(`GET /repositories/${repo}/actions/summary`);
        const summary = await githubActionsService.getBuildStatusSummary(repo, branch);
        res.json(summary);
    }));
    /**
     ...
     * Re-run a workflow
     */
    router.post('/repositories/:repo/actions/runs/:runId/rerun', asyncHandler(async (req, res) => {
        const { repo, runId } = req.params;
     ...
     */
    router.post('/repositories/:repo/actions/runs/:runId/rerun-failed', asyncHandler(async (req, res) => {
        const { repo, runId } = req.params;
     ...
     */
    router.post('/repositories/:repo/actions/runs/:runId/cancel', asyncHandler(async (req, res) => {
        const { repo, runId } = req.params;
     ...
     */
    router.post('/repositories/:repo/actions/workflows/:workflowId/dispatches', asyncHandler(async (req, res) => {
    ...
    router.post('/permissions/check', asyncHandler(async (req, res) => {
        ...
        for (const permission of permissions) {
            results[permission] = permissionService.hasPermission(context, permission);
        }
        res.json({
            userId: context.userId,
            roles: context.roles,
     ...
     */
    router.get('/permissions/roles', asyncHandler(async (req, res) => {
     ...
     */
    router.get('/permissions/all', requireRole(permissionService, Role.ADMIN), asyncHandler(async (req, res) => {
        res.json({
     ...
     */
    router.get('/maturity/:owner/:repo', asyncHandler(async (req, res) => {
     ...
     */
    router.get('/maturity/:owner/:repo/badge', asyncHandler(async (req, res) => {
        const { owner, repo } = req.params;
    ...
    router.get('/cost/summary', asyncHandler(async (req, res) => {
        ...
        const summary = await costService.getCostSummary(period, service);
        res.json(summary);
    }));
    /**
     ...
     */
    router.get('/cost/recommendations', asyncHandler(async (req, res) => {
    ...
    router.get('/cost/forecast', asyncHandler(async (req, res) => {
        ...
        const forecast = await costService.getForecast(parseInt(months, 10));
        res.json(forecast);
    }));
    // ==========================================
    // AI Search Endpoints
    ...
    const aiSearchService = new AISearchService(config, logger);
     ...
     */
    router.post('/search/ask', asyncHandler(async (req, res) => {
     ...
     */
    router.post('/search/index', asyncHandler(async (req, res) => {
     ...
     */
    router.get('/search/stats', asyncHandler(async (req, res) => {
    ...
    const day2OpsService = new Day2OperationsService(config, logger);
    logger.info('Day-2 Operations Service initialized');
    /**
     * GET /operations/definitions
     * Get available operation definitions
     */
    router.get('/operations/definitions', asyncHandler(async (req, res) => {
     ...
     */
    router.get('/operations/definition/:type', asyncHandler(async (req, res) => {
        const { type } = req.params;
        logger.info(`GET /operations/definition/${type}`);
        const definition = day2OpsService.getOperationDefinition(type);
        if (!definition) {
     ...
     */
    router.post('/operations/execute', asyncHandler(async (req, res) => {
     ...
     */
    router.post('/operations/:id/approve', asyncHandler(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.post('/operations/:id/cancel', asyncHandler(async (req, res) => {
        const { id } = req.params;
     ...
     */
    router.get('/operations/history', asyncHandler(async (req, res) => {
        ...
        if (namespace || name) {
            filteredHistory = history.filter(op => (!namespace || op.target.namespace === namespace) &&
