...
export class ArgoCDService {
    ...
    async listApplications(filter) {
        ...
        try {
            ...
            throw new ArgoCDError(`Failed to list ArgoCD applications: ${error.message}`, error.response?.status || 500, error);
        }
    }
    /**
    ...
    async getApplication(appName) {
        ...
        try {
            ...
            throw new ArgoCDError(`Failed to get ArgoCD application ${appName}: ${error.message}`, error.response?.status || 500, error);
        }
    }
    /**
     * Sync an ArgoCD application
     */
    async syncApplication(appName, syncRequest) {
        if (this.useMockData) {
            return this.getMockSyncResponse(appName);
        }
        try {
            const response = await this.client.post(`/api/v1/applications/${appName}/sync`, {
            ...
            throw new ArgoCDError(`Failed to sync ArgoCD application ${appName}: ${error.message}`, error.response?.status || 500, error);
        }
    }
    /**
     * Sync multiple applications
     */
    async syncApplications(appNames, syncRequest) {
        const results = [];
     ...
     */
    async getApplicationsByBranch(branch) {
        ...
        return allApps.filter(app => app.metadata.name.startsWith(branch) &&
            app.spec.source.targetRevision === branch);
    }
    /**
     * Get application health status
     */
    async getApplicationHealth(appName) {
    ...
    getMockApplications(filter) {
        ...
        if (filter) {
            return apps.filter(app => app.metadata.name.toLowerCase().includes(filter.toLowerCase()));
        }
        return apps;
    }
    getMockSyncResponse(appName) {
        return {
            application: appName,
            status: 'Syncing',
...
//# sourceMappingURL=ArgoCDService.js.map