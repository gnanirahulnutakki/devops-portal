...
export const GET = withTenantApiHandler(
  async (_request, ctx) => {
    try {
      ...
      // Validate baseUrl scheme so a malformed config can't yield unsafe hrefs.
      let safeBase = '';
      try {
      ...
      } catch { /* invalid URL → safeBase empty */ }

      const items = (result.items || []).map((p: any) => {
        const nm = encodeURIComponent(p.metadata?.name || '');
        return {
          ...
          externalUrl: safeBase ? `${safeBase}/settings/projects/${nm}` : '',
        };
      });
