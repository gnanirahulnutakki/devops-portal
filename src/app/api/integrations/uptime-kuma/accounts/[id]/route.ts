...
export const PATCH = withTenantApiHandler(
  async (request, ctx) => {
    ...
    const { name, enabled, url, apiKey } = validation.data;
    const credentialsPatch =
      url || apiKey ? ({ ...(url ? { url } : {}), ...(apiKey ? { apiKey } : {}) } as any) : undefined;

    const result = await updateCredentialById(ctx.tenant.organizationId, 'UPTIME_KUMA', id, {
      name,
      enabled,
      credentialsPatch,
    });
    if (!result.success) {
      return errorResponse(
        'UPTIME_KUMA_ACCOUNT_UPDATE_FAILED',
        result.error || 'Failed to update Uptime Kuma account',
        500
      );
    }
...
export const DELETE = withTenantApiHandler(
  async (request, ctx) => {
    ...
    const ok = await deleteCredentialById(ctx.tenant.organizationId, 'UPTIME_KUMA', id);
    if (!ok) return errorResponse('NOT_FOUND', 'Uptime Kuma account not found', 404);
    return successResponse({ success: true });
  },
  { rateLimit: 'general', requiredRole: 'ADMIN' }
);
