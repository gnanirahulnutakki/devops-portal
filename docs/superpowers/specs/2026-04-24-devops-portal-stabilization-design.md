...
6. **ESLint cleanup back to `--max-warnings 0`** — CI was unblocked by raising the cap to 100 in commit `2583c21`; 68 real warnings remain (46 unused-vars, 22 exhaustive-deps). Dedicated tech-debt PR should remove unused imports and either wrap callbacks in `useCallback` or add `eslint-disable-next-line` with a documented reason for the exhaustive-deps warnings.
