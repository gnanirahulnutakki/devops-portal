...
6. **ESLint cleanup back to `--max-warnings 0`** — partially done in commit `b2c98ef`. All 46 `@typescript-eslint/no-unused-vars` cleared; threshold tightened from 100 → 25. 22 `react-hooks/exhaustive-deps` warnings remain, all of the form "useEffect at line N calls `loadXxx()` which isn't in deps". Proper fix is to wrap each `loadXxx` in `useCallback` (so it's stable) and add to deps, or restructure the effect so it doesn't need the callback. Mechanical but touches ~18 files.
7. **Helm image-tag drift** — `values-saasops1.yaml` still specifies `tag: v2-llm-stack` while production runs `v2-openapi-4` and the CI workflow pushes `latest` + `<sha>` + `<date>` tags on main. Either sync `values-saasops1.yaml` to `tag: "latest"` (auto-updates with every push to main) or adopt a semver-tagged release discipline (CI would additionally push `$(jq -r .version package.json)`). Requires a deployment decision.
8. **9 remaining moderate Dependabot alerts** — all either false-positives or require breaking upstream changes npm can't auto-resolve:
