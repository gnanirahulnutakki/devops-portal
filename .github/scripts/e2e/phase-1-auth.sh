...
ACTUAL=$(curl -m 30 -sS -o /dev/null -w "%{http_code}" "${PORTAL_URL}/login")
if [[ "$ACTUAL" == "200" ]]; then
...
SESSION=$(curl -m 30 -fsS -b "$COOKIE_FILE" "${PORTAL_URL}/api/auth/session" 2>/dev/null)
...
ORG_COUNT=$(curl -m 30 -fsS -b "$COOKIE_FILE" "${PORTAL_URL}/api/organizations" 2>/dev/null \
...
NO_ORG=$(curl -m 30 -sS -o /dev/null -w "%{http_code}" -b "$COOKIE_FILE" "${PORTAL_URL}/api/clusters")
if [[ "$NO_ORG" == "400" ]]; then
...
WITH_ORG=$(curl -m 30 -sS -o /dev/null -w "%{http_code}" -b "$COOKIE_FILE" -H "x-organization-id: $ORG_ID" "${PORTAL_URL}/api/clusters")
...
CSRF=$(curl -m 30 -fsS "${PORTAL_URL}/api/auth/csrf" 2>/dev/null | jq -r '.csrfToken' 2>/dev/null)
...
PROVIDERS=$(curl -m 30 -fsS "${PORTAL_URL}/api/auth/providers" 2>/dev/null | jq 'keys | length' 2>/dev/null)
